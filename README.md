...
`google_maps_key` 문자열은 `local.properties` 또는 환경 변수에서 주입됩니다. 실제 지도를 표시하려면 다음 중 하나를 준비하세요.

1. 프로젝트 루트의 `local.properties`에 API 키를 추가
   ```properties
   MAPS_API_KEY=발급받은_구글맵_API_키
   ```
2. 또는 빌드 시 환경 변수로 설정
   ```bash
   export MAPS_API_KEY=발급받은_구글맵_API_키
   ```

유효한 키가 설정되지 않으면 구글 맵 영역이 검은색으로 표시될 수 있습니다.
